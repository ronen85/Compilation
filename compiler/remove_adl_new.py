...
def fixDomPredicates(dom):
    ...
    for pred in newPreds:
        if print_condition: print pred.asPDDL()
    return newPreds;
...
def fixActionConds(act):
    """
    input:       act: pddl.DurativeAction/pddl.Action
               domain: pddl.domain
    output: fixedConds: list of pddl.Formulas / pddl.Formula

    purpose : remove all negative condtions (e.g not(act ...) -> (isnt-act ...))
    """
    print_condition = True
...
if __name__ == "__main__":
    ...
    if make_reverse_negPred:
        print 'making a reverse predicate for all negPreds'
        negPreds = findNegPreds(dom)
        negPred = negPreds[0]
        revNegPred = makeReversePred(negPred)
